...
public class AutentificadorLogeo implements AutentificadorService, Serializable {
    ...
    public boolean login(String nombreUsuario, String claveUsuario) {
        ...
        if (!dato.getUsuLogin().equals(nombreUsuario) || !dato.getUsuContrasena().equals(claveUsuario)) {
            return false;
        }

        Session sess = Sessions.getCurrent();
        UserCredential cre = new UserCredential(dato, dato.getUsuLogin(), dato.getUsuContrasena(), dato.getUsuNivel(), dato.getUsuNombre());
